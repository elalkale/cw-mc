...
export default function ModpackDetail({ darkMode, onInstallStart, onInstallClear, installations = {} }: { darkMode: any; onInstallStart?: any; onInstallClear?: any; installations?: Record<string, any> }) {
  ...
  const openInstallModal = (fileId, fileLabel, defaultName, fileVersions: string[] = []) => {
    setInstallFileId(fileId);
    setInstallFileLabel(fileLabel);
    setServerNameInput(defaultName);
    setInstallFileVersions(fileVersions);
  ...
  const startInstall = async () => {
    ...
    // Use specific file's MC versions if available (from versions tab), else use modpack aggregate
      ...
      const res  = await fetchWithToken(`${API_BASE}/api/install`, {
        ...
        body:    JSON.stringify({
          ...
          meta: {
            ...
            modId:          localPack.modId,
            fileId,
            modpackVersion: installFileLabel ?? null,
