...
export default function ServerDetailPage({ servers, startServer, stopServer, sendCommand, darkMode, onBack }) {
  ...
  if (!data) {
    return (
      <div className="max-w-7xl mx-auto mt-6 p-4 md:p-6">
        ...
        <div className="bg-gradient-to-r from-red-900/30 to-pink-900/30 border border-red-500/50 text-red-300 px-4 py-3 rounded-lg">
          ⚠️ Servidor no encontrado
        </div>
      </div>
    );
  }
  ...
  return (
    <div className={`max-w-7xl mx-auto mt-2 md:mt-6 px-4 md:px-6 pb-8 transition-colors duration-300 space-y-4 ${
      darkMode ? "bg-gray-900" : "bg-gray-50"
    }`}>
      <button
                onClick={() => navigate('/dashboard')}
                className={`w-fit px-4 py-2 text-sm md:text-base rounded-lg transition border flex items-center gap-2 group ${
                  darkMode
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6">
        ...
        <div className="md:col-span-2 space-y-4 md:space-y-6">
          ...
          {/* Comandos rápidos */}
          <div className={`rounded-2xl shadow-lg p-4 md:p-6 border transition-colors ${
            darkMode
            ...
            <h2 className={`text-lg md:text-xl font-bold bg-clip-text text-transparent mb-4 ${
        ...
        <div className="md:col-span-1">
          <div className={`rounded-2xl shadow-lg p-4 md:p-6 border transition-colors ${
            ...
            <h3 className={`text-base md:text-lg font-bold bg-clip-text text-transparent mb-4 ${
              darkMode ? "bg-gradient-to-r from-purple-300 to-pink-300" : "bg-gradient-to-r from-purple-700 to-pink-700"
            }`}>
            ...
            <div className="flex flex-col gap-3">
              {data?.players?.sample?.length > 0 ? (
                data.players.sample.map((p) => (
                  <div key={p.id} className={`flex items-center gap-2 md:gap-3 p-2 rounded-lg border transition ${
                    darkMode
                    ...
                    />
                    <span className={`text-sm md:text-base truncate font-semibold ${
